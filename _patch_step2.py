...
# ============================================================
# 1) إنشاء ملف attendance/api_attendance_adjustment.py
# ============================================================
...
def _parse_time_str(time_str, record_date):
    ...
    # دعم صيغ مختلفة مثل HH:MM أو HH:MM:SS
...
def manager_adjust_attendance(request, attendance_id):
    \"\"\"
    تعديل سجل حضور وانصراف بواسطة HR / الإدارة
    POST /attendance/api/mobile/manager/attendance/<id>/adjust/
    Body:
    {
        "check_in_time": "09:00",
        "check_out_time": "17:30",
        "status": "present",
        "reason": "نسي تسجيل الانصراف وتم التحقق من الكاميرات"
    }
    \"\"\"
    user = request.user
    ...
    if not is_authorized:
        return Response({
            'success': False,
            'message': 'صلاحية غير كافية. تعديل الحضور متاح لـ HR والإدارة فقط'
        }, status=403)
    ...
    if not reason:
        return Response({
            'success': False,
            'message': 'يجب كتابة سبب التعديل بالتفصيل'
        }, status=400)
    ...
    if req_check_in is not None:
        ...
        if req_check_in:
            ...
            new_in = 'مسح'
        changes.append(f"وقت الحضور: {old_in} ➔ {new_in}")
    ...
    if req_check_out is not None:
        ...
        if req_check_out:
            ...
            new_out = req_check_out
        else:
            attendance.check_out_time = None
            new_out = 'مسح'
        changes.append(f"وقت الانصراف: {old_out} ➔ {new_out}")

    # تعديل الحالة
    if req_status and req_status in dict(Attendance.STATUS_CHOICES):
        old_status = attendance.status
        attendance.status = req_status
        changes.append(f"الحالة: {old_status} ➔ {req_status}")

    # إعادة احتساب ساعات العمل
    ...
    audit_msg = f"تعديل بواسطة ({user.get_full_name() or user.username}): {', '.join(changes)} | السبب: {reason}"
    attendance.admin_notes = f"{attendance.admin_notes}\n[{timezone.now().strftime('%Y-%m-%d %H:%M')}] {audit_msg}".strip()
    attendance.save()

    # تسجيل في سجل حركات الموظف EmployeeMovement
    try:
...
api_file = BASE / 'attendance' / 'api_attendance_adjustment.py'
api_file.write_text(api_code, encoding='utf-8')
print("[OK] Created attendance/api_attendance_adjustment.py")

# ============================================================
# 2) إضافة المسار في attendance/urls.py
# ============================================================
urls_file = BASE / 'attendance' / 'urls.py'
urls_content = urls_file.read_text(encoding='utf-8')

if 'manager_adjust_attendance' not in urls_content:
    import_stmt = "from .api_attendance_adjustment import manager_adjust_attendance\n"
    route_stmt = "    path('api/mobile/manager/attendance/<int:attendance_id>/adjust/', manager_adjust_attendance, name='manager_adjust_attendance'),\n"
    
    urls_content = import_stmt + urls_content
    # إضافته داخل urlpatterns
    pattern_index = urls_content.find('urlpatterns = [')
    if pattern_index != -1:
        bracket_end = urls_content.find('[', pattern_index) + 1
        urls_content = urls_content[:bracket_end] + '\n' + route_stmt + urls_content[bracket_end:]
        urls_file.write_text(urls_content, encoding='utf-8')
        print("[OK] Registered manager_adjust_attendance endpoint in attendance/urls.py")
    else:
        print("[!] Could not find urlpatterns in urls.py")
else:
    print("[SKIP] URL route already exists in attendance/urls.py")
