"""
Patch 49b Fix8 — Arabic PDF Rendering for Employees Export

الهدف:
- حل مشكلة أن PDF الموظفين يخرج مربعات
- استبدال xhtml2pdf في employees/exports.py بمحرك ReportLab
- استخدام فونت عربي من Windows
- دعم Arabic reshaping + bidi

هذا الباتش:
- يحدّث employees/exports.py فقط
- يحافظ على Excel كما هو
- supersedes تنفيذ PDF القديم المعتمد على xhtml2pdf
"""
...
print("=" * 60)
print("Patch 49b Fix8 — Arabic PDF Rendering for Employees Export")
print("=" * 60)

# ────────────────────────────────────────────────────────────
# Step 0: backup
# ────────────────────────────────────────────────────────────
exports_path = os.path.join(BASE_DIR, "employees", "exports.py")
...
if os.path.exists(exports_path):
    shutil.copy2(
        exports_path,
        os.path.join(backup_dir, "employees_exports_before_patch_49b_fix8.py.bak")
    )
    print("✅ Backup created: _patches/_backups/employees_exports_before_patch_49b_fix8.py.bak")

# ────────────────────────────────────────────────────────────
# Step 1: install dependencies
# ────────────────────────────────────────────────────────────
print("\n📌 Step 1: تثبيت dependencies اللازمة للعربي في PDF")
...
for pkg in packages:
    ...
    if result.returncode == 0:
        ...
        print(f"   ⚠️ تعذر تثبيت {pkg}")
        print((result.stderr or "")[:200])

# ────────────────────────────────────────────────────────────
# Step 2: rewrite employees/exports.py
# ────────────────────────────────────────────────────────────
print("\n📌 Step 2: تحديث employees/exports.py")
...
def export_employees_excel(employees, user=None):
    ...
    row_num += 1
    ws.merge_cells(f'A{row_num}:{last_col}{row_num}')
    cell = ws.cell(row=row_num, column=1, value="MotionHR — HR in Motion | JS Solution")
...
def export_employees_pdf(employees, user=None):
    ...
    story.append(table)
    story.append(Spacer(1, 10))
    story.append(_p("MotionHR — HR in Motion | JS Solution", footer_style))
...
print("\n" + "=" * 60)
print("✅ Patch 49b Fix8 اكتمل")
print("=" * 60)
print("""
اللي اتعمل:
  ✅ تم استبدال PDF engine الخاص بالموظفين من xhtml2pdf إلى ReportLab
  ✅ تم إضافة Arabic reshaping باستخدام:
     - arabic-reshaper
     - python-bidi
  ✅ تم استخدام فونت عربي من Windows تلقائيًا (Tahoma / Arial ...)
  ✅ تم الإبقاء على Excel كما هو
  ✅ تم إنشاء backup قبل التعديل

مهم:
  ✅ هذا الباتش supersedes تنفيذ PDF القديم في employees/exports.py
  ✅ ممنوع الرجوع لاستخدام xhtml2pdf في PDF الموظفين لو الهدف دعم العربي بشكل صحيح
