...
def manager_adjust_attendance(request, attendance_id):
    ...
    audit_msg = f"تعديل بواسطة ({actor_name}): {', '.join(changes)} | السبب: {reason}"
    stamp = timezone.now().strftime('%Y-%m-%d %H:%M')
    existing_notes = attendance.admin_notes or ''
    attendance.admin_notes = (existing_notes + f"\n[{stamp}] {audit_msg}").strip()
    attendance.save()
...
pathlib.Path('attendance/api_attendance_adjustment.py').write_text(code, encoding='utf-8')
print("[OK] attendance/api_attendance_adjustment.py fixed.")
