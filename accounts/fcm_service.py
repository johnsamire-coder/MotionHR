...
def send_notification_to_managers(company, title, body, data=None, title_en=None, body_en=None, employee=None):
    """
    إرسال إشعار للمديرين في شركة معينة.
    لو تم تمرير employee، الإشعار يوصل للمدير المباشر بتاعه بس (+ الأدمن/HR اللي بياخدوا كل حاجة دايمًا).
    لو مفيش employee، يوصل لكل المديرين (السلوك القديم، للحالات العامة).
    """
    from accounts.models import User

    always_notified = User.objects.filter(
        company=company,
        role__in=['company_admin', 'hr_manager', 'super_admin']
    )

    if employee is not None:
        direct_manager_users = []
        if employee.direct_manager and employee.direct_manager.user:
            direct_manager_users = [employee.direct_manager.user_id]
        managers = (always_notified | User.objects.filter(id__in=direct_manager_users)).distinct()
    else:
...
def notify_attendance_adjusted(*args, **kwargs):
    """Fallback handler for attendance adjustment notification"""
    try:
        pass
    except Exception:
        pass

# Permanent fallback for any missing FCM notification function
def __getattr__(name):
    if name.startswith('notify_') or name.startswith('send_'):
